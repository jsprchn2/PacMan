...
from pacman_lives import PacManCounter
from scores import ScoreController, LevelTransition
...
class PacManPortalGame:
    ...
    def __init__(self):
        pygame.init()
        pygame.mixer.music.load('sound/bgm2.wav')
        ...
        self.ghost_sound_manager = SoundManager(sound_files=['ghost_blue.wav', 'pacman_eatghost.wav', 'ghost_normal.wav'],
                                                keys=['blue', 'eaten', 'std'],
    ...
    def run(self):
        ...
        e_loop = GameFunctions(loop_running=True, actions={pygame.MOUSEBUTTONDOWN: menu.check_buttons})
    ...
    def play_game(self):
        """Run the game's event loop, using an GameFunctions object"""
        e_loop = GameFunctions(loop_running=True, actions={**self.player.event_map, **self.actions})
        # game init signal
